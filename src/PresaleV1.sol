...
contract PresaleV1 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function calculatePrice(uint256 _amount) public view returns (uint256) {
        ...
        if (_amount + total > rounds[0][currentStep] || block.timestamp >= rounds[2][currentStep]) {
            ...
            if (block.timestamp >= rounds[2][currentStep]) {
                require(
                    rounds[0][currentStep] + _amount <= rounds[0][currentStep + 1],
                ...
                USDTAmount = tokenAmountForCurrentPrice * rounds[1][currentStep]
                    + (_amount - tokenAmountForCurrentPrice) * rounds[1][currentStep + 1];
            }
        } else {
     ...
     * @param stake boolean flag for token staking
     */
    function buyWithEth(uint256 amount, bool stake)
        external
        payable
        ...
        returns (bool)
    {
        ...
        if (total > rounds[0][currentStep] || block.timestamp >= rounds[2][currentStep]) {
            ...
            console.logUint(111111111111111111111111111111111111);
            uint256 unsoldTokens = total > rounds[0][currentStep] ? 0 : rounds[0][currentStep] - total - amount;
            remainingTokensTracker.push(unsoldTokens);
            currentStep += 1;
        }
     ...
     * @param stake boolean flag for token staking
     */
    function buyWithETHWert(address _user, uint256 _amount, bool stake)
        external
        payable
    ...
    function splitUSDTValue(uint256 _amount) internal {
        if (wallets.length == 0) {
            ...
            (bool success,) = address(USDTInterface).call(
                abi.encodeWithSignature("transferFrom(address,address,uint256)", _msgSender(), paymentWallet, _amount)
            );
            ...
            for (uint256 i = 0; i < wallets.length; i++) {
                ...
                (bool success,) = address(USDTInterface).call(
                    abi.encodeWithSignature(
                        "transferFrom(address,address,uint256)", _msgSender(), wallets[i], amountToTransfer
                    )
                );
                require(success, "Token payment failed");
                tempCalc += amountToTransfer;
     ...
     */
    function startClaim(uint256 _claimStart, uint256 noOfTokens, address _saleToken, address _stakingManagerAddress)
        external
        onlyOwner
        returns (bool)
    {
